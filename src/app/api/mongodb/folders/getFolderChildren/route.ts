...
export async function GET(req: Request): Promise<Response> {
	...
	if (folderId) {
		try {
			const children = await db.folders.getOne.children.folderId(folderId);
			if (!children) return new Response(JSON.stringify({ error: "No folder found." }), { status: 404 }); // HTTP status code 404 indicates a resource not found
			return new Response(JSON.stringify(children), { status: 200 }); // HTTP status code 200 indicates success
		} catch (error: unknown) {
